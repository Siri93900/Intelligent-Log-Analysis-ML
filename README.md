...
## Key Finding
Baseline models achieved high accuracy (~97%) but very low recall (~10%), meaning they missed most real anomalies. Applying class-weighting to address this imbalance raised recall to ~37%, roughly a 3.6-fold improvement, but at a significant cost to precision (dropping from ~72% to ~4%). This precision-recall tradeoff was confirmed independently across two separately constructed samples of the dataset. Naive Bayes could not be adjusted using class-weighting due to a scikit-learn implementation limitation. Full results and analysis are available in the dissertation and in the `results/` folder.
...
## Code Sources
Individual pipeline components were informed by publicly available reference implementations (see Section 3.9.2 of the dissertation for full disclosure). The overall pipeline design and experimental structure are original to this study.
...
## Status
Literature review, methodology justification, and full pipeline implementation are complete. All four supervised models have been trained and evaluated on two independently constructed samples, with and without class-weighting, showing a consistent precision-recall tradeoff. Results, notebook, visualisations, and supporting documentation are available in the results/, notebooks/, and docs/ folders. The full dissertation draft (Chapters 1-6) is complete, incorporating supervisor feedback on research approach justification, data representativeness, and report formatting. 
